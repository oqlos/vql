...
from vql.drawing.renderers.svg import SVGRenderer
from vql.renderers.base import VQLRendererAdapter, render_program
...
def render_to_png(program: VQLProgram, path: str, *, scale: float = 1.0) -> str:
    ...
    Requires the optional ``vql[png]`` extra, which pulls in ``cairosvg``.
    ...
    try:
        ...
        raise RuntimeError(
            "PNG export requires the 'vql[png]' extra. Install it with: "
            "pip install 'vql[png]'"
        ) from exc
