"""FastAPI app — text/json/protobuf DSL dispatch for VQL."""
...
from dsl2vql.schema_registry import all_schemas, schema_for_verb

from fastapi import FastAPI, Request
from fastapi.responses import JSONResponse, PlainTextResponse, Response

_PROTO_DIR = Path(__file__).resolve().parents[3] / "dsl2vql" / "proto" / "dsl2vql" / "v1"


def create_app() -> FastAPI:
    app = FastAPI(title="rest2vql", version="0.1.0")

    @app.get("/health")
    def health() -> dict[str, str]:
        return {"status": "ok", "service": "rest2vql"}
    ...
    def list_proto() -> JSONResponse:
        ...
        return JSONResponse({"package": "dsl2vql.v1", "files": files})

    @app.get("/v1/events")
    def list_events(file: str = "app.vql.json") -> JSONResponse:
