...
- **Languages**: python: 127, toml: 11, json: 9, shell: 8, yaml: 2
...
### packages.dsl2vql.src.dsl2vql.handlers.command
- **Functions**: 6
- **File**: `command.py`

### packages.uri2vql.src.uri2vql.nlp2uri
- **Functions**: 6
- **Classes**: 1
- **File**: `nlp2uri.py`

### examples.img2nl-vql-flow
- **Functions**: 6
- **File**: `img2nl-vql-flow.sh`
...
### src.vql.drawing.star_generator.StarGenerator.generate
- **Calls**: params.get, params.get, range, pts.append, pts.append, pts.append, math.cos, math.sin
...
### Flow 2: parse_svg_path
```
parse_svg_path [src.vql.drawing.svg_path_parser]
```
...
### Flow 8: _extract_shape_specific_params
```
...
### src.vql.schema.program.Anchor
> A named reference point on the canvas/object.
- **Methods**: 2
- **Key Methods**: src.vql.schema.program.Anchor.to_dict, src.vql.schema.program.Anchor.from_dict
...
Supports: M, L, H, V, C, Q, Z (absolute) and m, l, 
- **Output to**: re.findall, len, groups.append, min, max

### src.vql.validation.spec.validate_program
> Validate a program structurally and against a spec.

If ``spec`` is omitted, the program's own ``val
- **Output to**: list, VQLValidationReport, program.validate, src.vql.validation.spec._match_items, src.vql.validation.spec._match_items
...
- `src.vql.drawing.svg_path_parser.parse_svg_path` - 64 calls
...
- `packages.img2svg.src.img2svg.to_vql.trace_to_vql_program` - 35 calls
- `packages.dsl2vql.src.dsl2vql.events.EventStore.append` - 33 calls
- `packages.img2svg.src.img2svg.cli.main` - 30 calls
...
- `packages.img2svg.src.img2svg.svg_emit.image_to_svg` - 21 calls
...
- `packages.img2svg.src.img2svg.to_vql.image_to_vql` - 16 calls
- `packages.dsl2vql.src.dsl2vql.handlers.query.handle_render` - 16 calls
- `packages.img2vql.src.img2vql.metadata.rapidocr_special_hits` - 16 calls
- `packages.img2vql.src.img2vql.metadata.save_diagnose_to_program` - 16 calls
- `src.vql.drawing.nl_parser.NLDrawingParser.parse` - 16 calls
...
graph TD
    ...
    main --> add_parser
    main --> add_argument
    parse_svg_path --> findall
    parse_svg_path --> len
    parse_svg_path --> append
    parse_svg_path --> min
    parse_svg_path --> max
    dispatch --> strip
    dispatch --> startswith
